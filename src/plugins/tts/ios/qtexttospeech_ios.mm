...
void QTextToSpeechEngineIos::say(const QString &text)
{
    ...
    if (m_rate >= 0) {
        // The QtTextToSpeech documentation states that a rate of 0.0 represents normal speach flow.
        // To map that to AVSpeechUtteranceDefaultSpeechRate while at the same time preserve the Qt
        // scale [-1.0, 1.0], we choose to operate with two differente rate convertions; one for
